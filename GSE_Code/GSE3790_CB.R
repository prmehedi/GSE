...
write.table(pdata, "pdata_gse3790cb.txt", sep="\t",row.names = T)

fdata <- gse$`GSE3790-GPL96_series_matrix.txt.gz`@featureData@data

write.table(fdata,"fdata_gse3790cb.txt", sep="\t",row.names = T)


data <- read.delim("GSE3790_CB_avg.txt",header = T, row.names = 1)

tmp <- read.delim("pdata_gse3790cb.txt",header = T)
...
write.table(phdata,"phdata.txt", sep="\t", row.names = T) #check it is necessory or not

phedata <- phdata[c(137:202),]
